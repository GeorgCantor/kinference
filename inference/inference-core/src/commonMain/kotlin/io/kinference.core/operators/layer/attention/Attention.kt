...
sealed class Attention(name: String, info: OperatorInfo, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) : Operator<KITensor, KITensor>(name, info, attributes, inputs, outputs) {
    companion object {
        private suspend fun attentionScore(
            ...
            numHeads: Int, hiddenSize: Int, present: NDArrayCore, context: ArrayContext? = null
...
//                resultMarker = futureResMarker as Array<StateMarker>
        ...
        internal suspend fun getScores(
            ...
            past: NDArrayCore?, batchSize: Int, seqLen: Int, numHeads: Int, hiddenSize: Int, maskFilterValue: Float = -10_000f, context: ArrayContext? = null
        ): Pair<NDArrayCore, NDArrayCore> {
            val headSize = hiddenSize / numHeads
        ...
        private suspend fun normalizedScores(
            ...
            seqLen: Int, pastSeqLen: Int, headSize: Int, numHeads: Int, present: NDArrayCore, maskFilterValue: Float = -10_000f, context: ArrayContext? = null
        ): NumberNDArrayCore {
...
//            return scores.softmax(axis = -1)
        ...
        private suspend fun IntNDArray?.maskFromIndices(unidir: Boolean, batchSize: Int, seqLen: Int, pastSeqLen: Int, maskFilterValue: Float = -10_000f, arrayContext: ArrayContext? = null): FloatNDArray {
            ...
            val mask = arrayContext?.floatStorage?.getNDArray(maskStrides) ?: MutableFloatNDArray(maskStrides)
...
class AttentionVer1(name: String, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) : Attention(name, INFO, attributes, inputs, outputs) {
    companion object {
        ...
        internal suspend fun initQueryKeyValue(
            ...
            batchSize: Int, seqLen: Int, hiddenSize: Int, numHeads: Int, context: ArrayContext? = null
    ...
    override suspend fun <D : ONNXData<*, *>> apply(contexts: Contexts<D>, inputs: List<KITensor?>): List<KITensor?> {
        ...
        val (queries, keys, values) = initQueryKeyValue(
            ...
            preparedBias.data,
            batchSize, seqLen, hiddenSize, numHeads, arrayContext
        )

        val (scores, present) = getScores(unidir, queries, keys, values, maskIndices, past, batchSize, seqLen, numHeads, hiddenSize, maskFilterValue, arrayContext)
        return listOf(scores.asTensor(arrayContext = arrayContext), present.asTensor(arrayContext = arrayContext))
    }
}
